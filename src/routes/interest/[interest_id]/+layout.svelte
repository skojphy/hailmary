<script lang="ts">
	...
	let isShortsView = $derived(/shorts\/\w+$/.test(page.url.pathname));
</script>

<div
	class={`min-h-screen flex flex-col mx-auto w-full max-w-md shadow-2xl relative ${$themeColors.background}`}
